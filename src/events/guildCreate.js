...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Universe has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const UniverseColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (UniverseColor) await guild.me.roles.add(UniverseColor);
  } catch (err) {
    client.logger.error(err.message);
  }
